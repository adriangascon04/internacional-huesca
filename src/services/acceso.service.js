...
/**
 * Extrae el ID de socio del texto de un QR ("HUESCA:123" -> "123").
...
export function parseQr(raw) {
  ...
  return texto.toUpperCase().startsWith(QR_PREFIX)
 ...
 * @returns {{estado:'sin_jornada'|'bloqueada'|'desconocido'|'repetido'|'valido', socio?, hora?}}
 */
export async function procesarAcceso(rawId, jornadaKey, jornadaBloqueada) {
  const id = parseQr(rawId);
